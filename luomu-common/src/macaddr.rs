...
impl MacAddr {
    ...
    pub fn multicast_mac_for(addr: IpAddr) -> Self {
        if !addr.is_multicast() {
            // not a multicast address, send to broadcast (ipv4) or to
            // all-nodes multicast (ipv6)
            return match addr {
                IpAddr::V4(_) => MacAddr::BROADCAST,
                IpAddr::V6(_) => MacAddr::from(ALL_NODES_MAC),
            };
        }
        match addr {
            IpAddr::V4(a) => {
...
mod tests {
    ...
    fn test_create_multicast_mac_for4_unicast() {
        ...
        let mac = MacAddr::multicast_mac_for(addr);
